...
function appendMarkdownSegment(segments, text, isCode) {
  if (!text) return;
...
function splitFencedCodeSegments(content) {
  const segments = [];
  let inFence = false;
  let fenceChar = "";
  let fenceLength = 0;
  ...
  while (pos < content.length) {
    ...
    const nextPos = newlineIndex === -1 ? content.length : newlineIndex + 1;
    const line = content.slice(pos, nextPos);
    const lineText = line.replace(/\r?\n$/, "");

    if (inFence) {
      appendMarkdownSegment(segments, line, true);
      ...
      const openMatch = lineText.match(/^ {0,3}(`{3,}|~{3,})/);
      if (openMatch) {
        appendMarkdownSegment(segments, line, true);
        inFence = true;
        ...
        appendMarkdownSegment(segments, line, false);
      }
    }
...
function splitInlineCodeSegments(text) {
  ...
  while (pos < text.length) {
    ...
    if (start === -1) {
      appendMarkdownSegment(segments, text.slice(pos), false);
      break;
    }

    let tickEnd = start;
    while (text[tickEnd] === "`") tickEnd++;
    const ticks = text.slice(start, tickEnd);
    const end = text.indexOf(ticks, tickEnd);
    if (end === -1) {
      appendMarkdownSegment(segments, text.slice(pos), false);
      break;
    }

    appendMarkdownSegment(segments, text.slice(pos, start), false);
    appendMarkdownSegment(segments, text.slice(start, end + ticks.length), true);
    pos = end + ticks.length;
  }

  return segments;
}

function splitMarkdownCodeSegments(content) {
  const segments = [];

  for (const segment of splitFencedCodeSegments(content)) {
    if (segment.isCode) {
      appendMarkdownSegment(segments, segment.text, true);
    } else {
      for (const inlineSegment of splitInlineCodeSegments(segment.text)) {
        appendMarkdownSegment(segments, inlineSegment.text, inlineSegment.isCode);
      }
    }
  }
...
function extractLatexPlaceholders(content, placeholders) {
  return splitMarkdownCodeSegments(content)
    .map((segment) => {
      if (segment.isCode) return segment.text;

      let processed = segment.text.replace(/\$\$([\s\S]*?)\$\$/g, (match, tex) => {
...
function renderMarkdown(content) {
  ...
  // Extract LaTeX before markdown to preserve \\ and other sequences, but only
  // outside markdown code fences/spans so code remains literal.
  const placeholders = [];
  const processed = extractLatexPlaceholders(content, placeholders);
...
function getImageLabel(image, index = 0) {
  return image?.name || `Pasted image ${index + 1}`;
}

function renderImageAttachments(images, callbacks = {}) {
  if (!images || images.length === 0) return null;

  return (
    <div className="message-image-tray">
      {images.map((image, i) => (
        <img
          alt={getImageLabel(image, i)}
          className="message-image-thumb"
          key={image.id || i}
          onClick={() => callbacks.openPastedImage?.(image)}
          src={image.dataUrl}
          title={getImageLabel(image, i)}
        />
      ))}
    </div>
  );
 ...
 */
export function renderUserMessage(msg, index, callbacks = {}) {
  ...
  return (
    <div className="message message-user" key={index}>
      ...
      />
      {renderImageAttachments(msg.images, callbacks)}
    </div>
  );
}
...
export function renderStreamingMessage(currentText, currentThinking, isLoading) {
  ...
  if (isWaiting && !currentThinking) {
    return (
      <div className="response-sequence streaming-sequence processing-sequence">
        <div className="processing-marker loading-indicator">
...
function groupMessagesByUser(messages) {
  ...
  messages.forEach((msg, messageIndex) => {
    const item = { msg, messageIndex };
    if (msg.role === "user") {
      if (currentItems.length > 0) {
        groups.push({ type: "response", items: currentItems });
        currentItems = [];
      }
      groups.push({ type: "user", message: msg, messageIndex });
    } else {
 ...
 */
function renderUserMessageBlock(msg, index, callbacks = {}, messageIndex = index) {
  return (
    <div
      className="user-message-block"
      data-message-anchor={messageIndex}
      key={`user-${messageIndex}`}
    >
      {renderUserMessage(msg, index, callbacks)}
    </div>
  );
}
  ...
  return (
    <div className="response-sequence" key={`response-${groupIndex}`}>
      {items.map((item, i) => {
        ...
        const itemMessage = item.msg || item;
        const itemMessageIndex = item.messageIndex ?? i;
        ...
        const dotClass = getDotClass(itemMessage);

        return (
          <div
            className={`timeline-item ${isTimelineLast ? "timeline-last" : ""}`}
            data-message-anchor={itemMessageIndex}
            key={itemMessageIndex}
          >
            <div className={`timeline-dot ${dotClass}`}></div>
            <div className="timeline-line"></div>
            <div className="timeline-content">
              {renderTimelineItem(itemMessage, i, toolHandlers)}
            </div>
          </div>
        );
      })}
 ...
 */
export function renderMessages(messages, toolHandlers, isStreaming = false, callbacks = {}) {
  ...
  return groups.map((group, groupIndex) => {
    ...
    if (group.type === "user") {
      return renderUserMessageBlock(group.message, groupIndex, callbacks, group.messageIndex);
    } else {
        ...
        <div className="welcome-tips">
          ...
          <div className="tip-section">
            ...
            <ul>
              ...
              <li>
                <kbd>Ctrl+1</kbd> Ask permissions: Ask before actions
              </li>
              <li>
                <kbd>Ctrl+2</kbd> Accept edits: Auto-apply changes
              </li>
              <li>
                <kbd>Ctrl+3</kbd> Plan mode: Read-only
              </li>
              <li>
                <kbd>Ctrl+4</kbd> Auto mode: Background safety checks
              </li>
            </ul>
          </div>
