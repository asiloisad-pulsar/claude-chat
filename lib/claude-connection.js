    ...
    switch (event.type) {
      ...
      case "stream_event":
        ...
        if (event.event?.type) {
          ...
          this.emitter.emit("usage", {
            input: event.event.usage.input_tokens || 0,
            output: event.event.usage.output_tokens || 0,
            cacheRead: event.event.usage.cache_read_input_tokens || 0,
            cacheCreation: event.event.usage.cache_creation_input_tokens || 0,
          });
        }
        break;
