...
export const renderToolAskUser = createToolRenderer({
  ...
  getInfo: (input, result, msg) => {
    ...
    const answered = answers[first?.question];
    return {
      description: first?.question || input?.question || "",
      resultInfo: answered ? `\u2192 ${answered}` : undefined,
    };
  },
  hasExpandable: (input) => {
    const questions = input?.questions || [];
...
export const renderToolToolSearch = createSimpleToolRenderer("ToolSearch", (input) => ({
  code: input?.query,
  badge: input?.max_results ? `max ${input.max_results}` : null,
}));
