...
function createShellCommandRenderer(name, className) {
...
export const renderToolBash = createShellCommandRenderer("Bash", "bash");

export const renderToolPowerShell = createShellCommandRenderer("PowerShell", "powershell");
...
export function toolHasDetails(name, input, result) {
  const checks = {
    ...
    PowerShell: () => input?.command?.length > 60 || result,
    Powershell: () => input?.command?.length > 60 || result,
    powershell: () => input?.command?.length > 60 || result,
    ShellCommand: () => input?.command?.length > 60 || result,
    shell_command: () => input?.command?.length > 60 || result,
...
const TOOL_RENDERERS = {
  ...
  Edit: renderToolEdit,
  Bash: renderToolBash,
  PowerShell: renderToolPowerShell,
  Powershell: renderToolPowerShell,
  powershell: renderToolPowerShell,
  ShellCommand: renderToolPowerShell,
  shell_command: renderToolPowerShell,
...
export function renderTool(msg, index, handlers) {
  ...
  if (msg.input?.command) {
    return renderToolPowerShell(msg, index, handlers);
  }
