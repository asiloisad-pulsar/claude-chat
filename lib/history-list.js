    ...
    super({
      ...
      listCommands: {
        ...
        "claude-chat:delete-session": async () => {
          const item = self.instance.getSelectedItem();
          if (item) {
            const selectedIndex = self.instance.getSelectedIndex();
            await deleteSession(item.sessionId);
            self.items = self.items.filter((i) => i.sessionId !== item.sessionId);
            await self.instance.updateItems(self.items);
            const visibleCount = self.instance.getVisibleItemCount();
            if (visibleCount > 0) {
              await self.instance.selectIndex(Math.min(selectedIndex ?? 0, visibleCount - 1));
            }
          }
        },
  ...
  async loadItems() {
    ...
    if (state.reload) {
      this.update({
        ...
        helpMarkdown:
          (state.showAll ? "Showing: **All sessions**" : "Showing: **Project sessions**") +
          "\n\n" +
          "Available commands:\n" +
          "- **Enter**: Open selected chat\n" +
          "- **Ctrl+Enter**: Open chat (keep list open)\n" +
          "- **Ctrl+D**: Delete selected chat\n" +
          "- **Ctrl+0**: Toggle project/all\n" +
          "- **F5**: Refresh list",
      });
