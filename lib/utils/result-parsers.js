...
export function parseJsonResult(result) {
  ...
  try {
    ...
    const parseContentArray = (content) => {
      const textBlock = content.find((b) => b.type === "text" && b.text);
      return textBlock ? parseText(textBlock.text) : content;
    };

    // Handle MCP content array format: [{type: "text", text: "..."}]
    if (Array.isArray(result)) {
      return parseContentArray(result);
    }

    // Handle direct JSON string
    const parsed = parseText(result);

    // Handle MCP CallToolResult format: {content: [{type: "text", text: "..."}], isError}
    if (Array.isArray(parsed?.content)) {
      return parseContentArray(parsed.content);
    }
