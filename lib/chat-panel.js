...
export default class ChatPanel {
  ...
  constructor(props = {}) {
    ...
    // Effort mode for this chat
    this.effortMode = props.effortMode || Config.effortMode();
   ...
   * (avoids tiny fragment thinking blocks from interleaved content blocks).
   */
  finalizeThinking() {
    if (!this.currentThinking) return;
    const last = this.messages[this.messages.length - 1];
    if (last?.role === "thinking") {
      last.content += this.currentThinking;
    } else {
      this.addMessage("thinking", this.currentThinking, {
  ...
  updateTooltips() {
    ...
    if (this.refs.effortMode) {
      const current = Config.effortModes.find((m) => m.value === this.effortMode) || Config.effortModes[1];
      this.tooltipDisposables.add(
        atom.tooltips.add(this.refs.effortMode, {
  ...
  scheduleUpdate() {
    ...
    requestAnimationFrame(() => {
      this.updateScheduled = false;
      // Skip if response already finalized (result flushed buffers)
      ...
      if (this.pendingDelta && this.currentThinking) {
        this.finalizeThinking();
      }
      if (this.pendingDelta) {
        this.currentText += this.pendingDelta;
  ...
  cycleEffortMode() {
    ...
    const currentIndex = modes.findIndex((m) => m.value === this.effortMode);
    const nextIndex = (currentIndex + 1) % modes.length;
    this.effortMode = modes[nextIndex].value;
    if (this.connection.isRunning()) {
      this.connection.setEffortMode(this.effortMode);
    }
    this.update();
  }
  ...
  renderEffortMode() {
    const modes = Config.effortModes;
    const current = modes.find((m) => m.value === this.effortMode) || modes[1];
    const activeDots = current.dots;

    return (
      <div
        className={`effort-mode effort-${current.value}`}
        ref="effortMode"
        on={{ click: () => this.cycleEffortMode() }}
      >
        {modes.map((_, i) => (
          <span className={`effort-dot${i < activeDots ? " active" : ""}`} />
        ))}
      </div>
    );
  }
