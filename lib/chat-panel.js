...
export default class ChatPanel {
  ...
  teardownAutoScroll() {
    ...
    if (this._onMessagesScroll && this._refs.messagesContainer) {
      this._refs.messagesContainer.removeEventListener("scroll", this._onMessagesScroll);
    }
    this._onMessagesScroll = null;
  }
...
function ChatApp({ signals, callbacks, promptEditorElementRef }) {
  ...
  const hasStreamingTimeline = !!(signals.currentText.value || signals.currentThinking.value);
  const isBusy = signals.isLoading.value || hasStreamingTimeline;
  const isEmpty =
    ...
    <div className="claude-chat" tabIndex="-1">
      <div className="claude-chat-messages" ref={messagesContainerRef}>
        <div className="claude-chat-messages-content" ref={messagesContentRef}>
          ...
          {!isEmpty
            ? renderMessages(signals.messages.value, callbacks.toolHandlers, hasStreamingTimeline, {
