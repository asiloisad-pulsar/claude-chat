...
export default class ChatPanel {
  ...
  createQueuedMessage(content, attach = []) {
    return {
  ...
  queueMessage(content, attach = []) {
    if (!content && attach.length === 0) return false;

    const queuedMessage = this.createQueuedMessage(content, attach);
    const wasNearBottom = this.isNearBottom();
  ...
  sendMessageNow({ content = "", attach = [], timestamp = new Date().toISOString() }) {
    if (!content && attach.length === 0) return false;

    const attachPrefix = this.formatAttachContext(attach);
    const fullMessage = attachPrefix + content;
  ...
  steerQueuedMessage(id) {
    ...
    const fullMessage = this.formatAttachContext(attach) + content;
  ...
  sendPrompt(text, attachContext = null) {
    ...
    if (this.isTurnBusy()) {
      const queued = this.queueMessage(text, attachments);
      if (queued) this.signals.attachContext.value = [];
      return queued;
    }

    this.signals.attachContext.value = [];
    this.sendMessageNow({ content: text, attach: attachments });
    return true;
  }
  ...
  handleSend() {
    ...
    this.signals.attachContext.value = [];
    if (this.isTurnBusy()) {
      this.queueMessage(text, attachments);
    } else {
      this.sendMessageNow({ content: text, attach: attachments });
    }
    this.focus();
