...
export default class ChatPanel {
  ...
  updateTooltips() {
    ...
    if (this._refs.attachIndicator && attachCtx) {
      ...
      if (attachCtx.type === "selections" && attachCtx.selections) {
        ...
        if (hasText) {
          const totalChars = attachCtx.selections.reduce(
  ...
  formatAttachContext() {
    ...
    if (type === "selections" && selections) {
      ...
      const lines = ["<attachments>", "<attachment>", "type: image", `path: ${file}`];
      if (dimensions) {
...
function ChatApp({ signals, callbacks, promptEditorElementRef }) {
  ...
  const showStop =
    signals.isLoading.value && !signals.hasEditorInput.value && !signals.attachContext.value;
      ...
      <div className="claude-chat-input">
        ...
        <div className="claude-chat-toolbar">
          {signals.attachContext.value ? (
            ...
            >
              <span className={`icon icon-${signals.attachContext.value.icon || "mention"}`}></span>
