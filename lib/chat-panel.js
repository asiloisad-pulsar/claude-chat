...
export default class ChatPanel {
  ...
  removeAttachContext(index) {
    this.signals.attachContext.value = this.signals.attachContext.value.filter((_, i) => i !== index);
  }
...
function ChatApp({ signals, callbacks, promptEditorElementRef }) {
  ...
  const showStop =
    signals.isLoading.value && !signals.hasEditorInput.value && signals.attachContext.value.length === 0;
          ...
          <div className="attach-tray">
            {signals.attachContext.value.map((ctx, i) => {
              ...
              return (
                ...
                >
                  <span className={["icon", ...(ctx.iconClasses || [`icon-${ctx.icon || "mention"}`])].join(" ")}></span>
