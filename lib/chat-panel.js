...
export default class ChatPanel {
  ...
  setupConnection() {
    ...
    this.disposables.add(
      this.connection.on("result", (resultText) => {
        // Measure before finalizing: finalizing swaps the streaming element for the
        // rendered message and removes the loading indicator, changing the layout.
        const wasNearBottom = this.isNearBottom();
        ...
        this.signals.isLoading.value = false;
        this.stopRequested = false;
        if (wasNearBottom) this.scrollToBottomAfterRender();
  ...
  // Scroll after Preact has re-rendered and laid out. Used when the view changes
  // shape (e.g. finalizing a stream swaps the streaming element for a taller
  // rendered message), where a microtask would scroll against the stale layout.
  ...
  setAttachContext(context) {
    const existing = this.signals.attachContext.value;
    ...
    this.signals.attachContext.value = [...existing, context];
  }
  ...
  sendNextQueuedMessage() {
    ...
    this.signals.queuedMessages.value = queuedMessages.filter((_, i) => i !== nextIndex);
    return this.sendMessageNow(nextMessage);
  }

  toggleQueuedMessageFreeze(id) {
    this.signals.queuedMessages.value = this.signals.queuedMessages.value.map((message) =>
