...
/**
 * Convert permission_suggestions array to a human-readable button label.
 * Returns null if no suggestions or not displayable.
 */
function getSuggestionLabel(suggestions) {
  if (!suggestions?.length) return null;
  const s = suggestions[0];
  if (s.type === "addRules" && s.behavior === "allow") {
    return "Always Allow";
  }
  if (s.type === "setMode" && MODE_LABELS[s.mode]) {
    return MODE_LABELS[s.mode] + " Mode";
  }
  return null;
}
...
export default class ChatPanel {
  ...
  handlePermissionAccept(withSuggestions = false) {
    if (!this.pendingPermission) return;
    const { requestId, input, suggestions } = this.pendingPermission;
    const opts = { input };
    if (withSuggestions && suggestions?.length) {
      opts.permissions = suggestions;
    }
  ...
  /**
   * Handle answering an AskUserQuestion prompt
   * @param {string} questionText - The question being answered
   * @param {string} answer - The selected option label
   */
  handleQuestionAnswer(questionText, answer) {
    if (!this.pendingPermission) return;
    const { requestId, input } = this.pendingPermission;

    // Build answers map — answer all questions (use first option for unselected)
    const answers = {};
    ...
    // Spread original input, add answers
    const updatedInput = { ...input, answers };
    log.debug("Question answer", { questionText, answer, updatedInput });
  ...
  handleQuestionAnswerOther(questionText) {
    const text = this.promptEditor?.getText()?.trim();
    if (!text) {
      // Focus the editor so user can type their answer
      this.focus();
      return;
    }
    this.promptEditor.setText("");
  ...
  renderPermissionPrompt() {
    ...
    // Build suggestion label from permission_suggestions
    const suggestionLabel = getSuggestionLabel(suggestions);
        ...
        <div className="permission-prompt-actions">
          ...
          {suggestionLabel ? (
            <button
              ...
              on={{ click: () => this.handlePermissionAccept(true) }}
            >
              {suggestionLabel}
            </button>
          ) : null}
